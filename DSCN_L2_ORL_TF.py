...
class ConvAE(object):
    ...
    def partial_fit(self, X, lr): #  
        cost, summary, _, Coef, reconst_cost, selfexpress_losses = self.sess.run((self.reconst_cost, self.merged_summary_op, self.optimizer, self.Coef, self.reconst_cost, self.selfexpress_losses), feed_dict = {self.x: X, self.learning_rate: lr})#
        self.iter = self.iter + 1
        return cost, Coef, reconst_cost, selfexpress_losses
...
def test_face(Img, Label, CAE, num_class):       
    ...
    for i in range(0,41-num_class): 
        ...
        while epoch < max_step:
            epoch = epoch + 1           
            cost, Coef, recons_loss, self_expressloss = CAE.partial_fit(face_10_subjs, lr)#                                  
            if epoch % display_step == 0:
                print "epoch: %.1d" % epoch, "cost: %.8f" % (cost/float(batch_size))  
                print(np.square(Coef).sum())
                print("Recon loss : ", recons_loss/400)
                print("Self-express loss : ", self_expressloss)                
...
if __name__ == '__main__':
    ...
    while iter_loop < len(all_subjects):
        ...
        model_path = './DSCN_L2_ORL.pt' 
        restore_path = '../Deep-subspace-clustering-networks/pretrain-model-ORL/model-335-32x32-orl.ckpt' 
